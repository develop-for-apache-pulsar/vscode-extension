...
export default class WatchFunctionDeletingTask implements TObservableTask<FunctionStatus> {
  ...
  errorThreshold = 3;
  messages = {
    startingTask: "Deleting function",
  ...
  onProgress = (functionStatus?: FunctionStatus) => {
    ...
    return new class implements ProgressReport {
      message = "Function being deleted";
      increment = increment;
    };
  };
  ...
  onFinish = (waitExpired:boolean, wasCancelled: boolean, hasErrors: boolean): void => {
    ...
    window.showInformationMessage(`Function ${this.tenantName}/${this.namespaceName}/${this.functionName} deleted`);
  };
  hasErrors = () => {
    return false;
  };
